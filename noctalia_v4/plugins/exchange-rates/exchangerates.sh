...
if [ -n "$today_entry" ] && [ "$force" -eq 0 ]; then
    ...
    if [ "$cache_hour" -gt 9 ] || { [ "$cache_hour" -eq 9 ] && [ "$cache_min" -ge 0 ]; }; then
        echo "{\"text\": \"USD: $cache_usd | EUR: $cache_eur\", \"tooltip\": \"$(get_last_seven_entries)\", \"percentage\": 100}"
        exit 0
    fi
fi
...
if [ -n "$last_entry" ]; then
    ...
    if [ -z "$eur" ] || [ -z "$usd" ]; then
        echo "{\"text\": \"USD: $cache_usd | EUR: $cache_eur\", \"tooltip\": \"$(get_last_seven_entries)\", \"percentage\": 0}"
    ...
    if [ "$eur" != "$cache_eur" ] || [ "$usd" != "$cache_usd" ] || [ "$force" -eq 1 ]; then
        ...
        notify-send -i "$icon" -u critical "Exchange rates" "USD: $cache_usd → $usd<br>EUR: $cache_eur → $eur"
    fi
fi
...
echo "{\"text\": \"USD: $usd | EUR: $eur\", \"tooltip\": \"$(get_last_seven_entries)\", \"percentage\": 50}"
