-- Copy of built-in vim.lsp.completion, modified
local M = {}
...
function M._lsp_to_complete_items(result, prefix, client_id)
  ...
  if not prefix:find('%w') then
    ...
    matches = function(item)
      ...
      local match = vim.fn.matchfuzzypos({ text }, prefix)
      if #match[1] > 0 then
        return match[3][1]
      end
      return false
  ...
  for _, item in ipairs(items) do
    local match_score = matches(item)
    if match_score then
      ...
      local completion_item = {
        ...
        fuzzy_sort = type(match_score) == 'number',
        user_data = {
          nvim = {
            lsp = {
  ...
  table.sort(candidates, function(a, b)
    if a.fuzzy_sort and b.fuzzy_sort then
    ...
    for client_id, response in pairs(responses) do
      ...
      if result then
        ...
        client_matches, server_start_boundary =
          M._convert_results(line, cursor_row - 1, cursor_col, client_id, word_boundary, nil, result, encoding)
...
local function enable_completions(client_id, bufnr, opts)
  ...
  if not buf_handle then
    ...
    -- Set up autocommands.
    local group = api.nvim_create_augroup(string.format('vim/lsp/completion-%d', bufnr), { clear = true })
