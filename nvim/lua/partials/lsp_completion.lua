-- This is a copy of vim.lsp.completion, with some modifications to make it work automatically
local M = {}
...
local function trigger(bufnr, clients)
  ...
  print('trigger > line_to_cursor', vim.inspect(line_to_cursor))
  local word_boundary = vim.fn.match(line_to_cursor, '\\k*$')
  print('trigger > word_boundary', vim.inspect(word_boundary))
  ...
  local cancel_request = request(clients, bufnr, win, function(responses)
    ...
    if #matches == 0 and vim.fn.match(line_to_cursor, '\\k\\+$') > 2 then
